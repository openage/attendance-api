...
const getComesLate = (attendance, shiftType) => {
    ...
    const differenceInMin = moment(attendance.checkIn).diff(shiftStartTime, 'minutes')
    const differenceInHr = moment(attendance.checkIn).diff(shiftStartTime, 'hours')
    return (differenceInMin < 0) ? 'NA' : `${differenceInHr} : ${differenceInMin % 60}`
}

const getCheckIn = (attendance) => {
    return `${attendance.checkIn ? moment(attendance.checkIn).format('HH:mm') : 'NA'}`
}
...
const build = async (fileName, ofDate, getExtraHours, employees, orgDetails) => {
    ...
    await Promise.each(employees, async (employee) => {
        ...
        for (var date = 1; date <= moment(ofDate).daysInMonth(); date++) {
            ...
            if (attendance) {
                const isLate = getComesLate(attendance, attendance.shift.shiftType)
                if (isLate === 'NA') {
                    columnNo = 1
                } else {
                    ...
                    sheet1.align(columnNo, rowNo, 'center')
                    sheet1.border(columnNo, rowNo, { left: 'thin', top: 'thin', right: 'thin', bottom: 'thin' })
                    sheet1.set(columnNo++, rowNo, getCheckIn(attendance))
                    sheet1.width(columnNo, 12.0)
                    sheet1.font(columnNo, rowNo, { sz: '10' })
                    sheet1.align(columnNo, rowNo, 'center')
                    sheet1.border(columnNo, rowNo, { left: 'thin', top: 'thin', right: 'thin', bottom: 'thin' })
                    sheet1.set(columnNo++, rowNo, isLate)
                    columnNo = 1
                    rowNo++
                    serialNo++
                    addRow = true
