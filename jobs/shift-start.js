...
const logger = require('@open-age/logger')('JOB shift-start')
...
var start = async (shiftType, organization, date) => {
    ...
    await offline.queue('shift', 'create', {
        shiftType: shiftType,
        date: date
    }, context)
...
exports.schedule = () => {
    ...
    db.shiftType.find({}).populate('organization').then(shiftTypes =>
        bluebird.each(shiftTypes, shiftType => {
            let startJobHour = moment(shiftType.startTime).subtract(1, 'hours').get('hour')
            let startJobMin = moment(shiftType.startTime).get('minute')
            ...
            log.info(`scheduled: shift start for '${shiftType.code}' of '${shiftType.organization.code}'`)
        })
            .then(() => {
                log.end()
            })
